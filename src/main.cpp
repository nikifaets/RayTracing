...
int main(int argc, char * argv[]){
    ...
    Vector3D light_direction(1,1,0);
    DirectionalLight light(light_direction);

	Sphere sphere(Vector3D(900,0,0), 400);
    Sphere sphere1(Vector3D(320, 300, 0), 100);
    Sphere sphere2(Vector3D(200, 100, 200), 100);
    ...
    for(int i=0; i<10; i++){
       ...
       // sphere1.translation = sphere1.translation + Vector3D(cos(i/10), -sin(i/10), sin(i/10)/3).scale(30);
        ...
        light.direction  = light.direction + Vector3D(0,-1,0);
        cout << "frame " << endl;
        scene.render();
    	window.render_display();
